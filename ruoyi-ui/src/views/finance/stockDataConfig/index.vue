    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['finance:stockDataConfig:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['finance:stockDataConfig:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['finance:stockDataConfig:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['finance:stockDataConfig:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listStockDataConfig, getStockDataConfig, delStockDataConfig, addStockDataConfig, updateStockDataConfig, exportStockDataConfig } from "@/api/finance/stockDataConfig";

export default {
  name: "StockDataConfig",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      listStockDataConfig(this.queryParams).then(response => {
        this.stockDataConfigList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateStockDataConfig(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addStockDataConfig(this.form).then(response => {
