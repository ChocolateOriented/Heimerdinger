...
public class WorkflowLeaveController extends BaseController {
    ...
    @GetMapping("/export")
    public Rest export(WorkflowLeave workflowLeave) {
        List<WorkflowLeave> list = workflowLeaveService.selectWorkflowLeaveList(workflowLeave);
    ...
    @GetMapping(value = "/{id}")
    public Rest getInfo(@PathVariable("id") String id) {
        return Rest.success(workflowLeaveService.selectWorkflowLeaveById(id));
    }   /**
     * 获取请假详细信息
     */
    @PreAuthorize("@ss.hasPermi('workflow:leave:query')")
    @GetMapping(value = "ByInstanceId/{instanceId}")
    public Rest getInfoByInstanceId(@PathVariable("instanceId") String instanceId) {
        return Rest.success(workflowLeaveService.selectWorkflowLeaveByInstanceId(instanceId));
    }
    ...
    @PostMapping
    public Rest add(@RequestBody WorkflowLeave workflowLeave) {
        logger.info("进入请假........");
    ...
    @PutMapping
    public Rest edit(@RequestBody WorkflowLeave workflowLeave) {
        return toAjax(workflowLeaveService.insertWorkflowLeave(workflowLeave));
    }
    ...
    @DeleteMapping("/{ids}")
    public Rest remove(@PathVariable String[] ids) {
